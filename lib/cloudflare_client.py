...
class CloudFlareClient:
    """
    CloudFlare客户端，可以获取和推送域名信息，可以存储key，域名ip等信息

    `CloudFlare DDNS <https://github.com/flytutu-susu/CloudFlare-DDNS>`_

    :author: flytutu-susu
    :changelog: 20xx-xx-xx: xxx
                2024-07-29 定制cloudflare版本
    :since: 2024-07-29
    """
    _API_BASE_URL = 'https://api.cloudflare.com/client/v4/zones'
    ...
    def __init__(self, http_client: httpx.Client, conf: dict) -> None:
        ...
        self._api_id = conf['id']
        self._api_key = conf['key']
        self._api_zone_id = conf['zone_id']
        ...
        if self.enable_ipv4:
            for i in domains_v4:
                self.domains.append(CloudFlareClient._separate(i if is_list else domains_v4))
                if not is_list:
                    break
        if self.enable_ipv6:
            for i in domains_v6:
                self.domains_ipv6.append(CloudFlareClient._separate(i))
    ...
    def _list_dns_api(self, domain: dict, cache: dict = {}, t: str = 'A') -> None:
        ...
        try:
            ...
            if ro is None:
                url = f"/{self._api_zone_id}/dns_records?order=type&direction=asc"
                ro = self._http_client.get(url, headers = {
            ...
            for record in r:
                if record['name'] == _domain and record['type'] == t:
                    r = record
                    break
            if type(r) == list:
                # 上方循环到最后也未匹配，未赋值
            ...
            domain['record_id'] = r['id']
            domain['domain_ip'] = r['content']
            self._logger.info(
    ...
    def update_domain_ip(self, new_ip=None, new_ipv6=None) -> int:
        ...
        if self.enable_ipv4 and new_ip is not None:
            r1 = self._update_dns_api(self.domains, new_ip, t = 'A')
        if self.enable_ipv6 and new_ipv6 is not None:
            r2 = self._update_dns_api(self.domains_ipv6, new_ipv6, t='AAAA')
        # 避免结果一正一负
        return r1 + r2 if r1 * r2 >= 0 else -1 * abs(r1 - r2)

    def _update_dns_api(self, domains: list, new_ip, t: str = 'A') -> int:
        """
        ...
        for domain in domains:
            ...
            try:
                url = f"/{self._api_zone_id}/dns_records/{domain['record_id']}"
                body = {
                    'type': t,
                    'name': full_domain,
                    'content': new_ip,
