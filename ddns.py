...
class DDNS:
    """
    `CloudFlare DDNS <https://github.com/flytutu-susu/CloudFlare-DDNS>`_

    :author: flytutu-susu
    :changelog: 20xx-xx-xx: xxx
                2024-07-29 定制支持CloudFlare
    :since: 2024-07-29
    """

    version = 'CloudFlare DDNS v1.0.0'
 ...
 ╚═════╝  ╚═════╝  ╚═╝  ╚═══╝ ╚══════╝
        ...
        self._cloudflare_client = CloudFlareClient(self._base_http_client, conf)
    ...
    def test_email(self):
        ...
        self._email_client.send_email('email_test', self._cloudflare_client.to_html_table())
    ...
    def start(self) -> None:
        ...
        开启循环
        """
        enable_ipv4 = self._cloudflare_client.enable_ipv4
        enable_ipv6 = self._cloudflare_client.enable_ipv6
        self._cloudflare_client.fetch_domains_info()
        error_count = 0
        current_ipv6 = None
        while True:
            try:
                ...
                if enable_ipv6:
                    ...
                    if current_ipv6 == '-1':
                        raise Exception('current_ip.fetch error')
                # 值得注意的是，当程序在运行一段时间后，而用户手动去Cloudflare修改了域名的解析值，由于程序只对比内存中的值，所以不会触发更新
                if (enable_ipv4 and not self._cloudflare_client.ip_equal(current_ip)) or \
                        (enable_ipv6 and not self._cloudflare_client.ip_equal_ipv6(current_ipv6)):
                    r = self._cloudflare_client.update_domain_ip(
                    ...
                    if r >= 0 and self._email_every_update:
                        self._email_client.send_email('update_successful', self._cloudflare_client.to_html_table(),
                                                      'new_ip', ip_msg)
                    else:
                        self._email_client.send_email('update_failed', self._cloudflare_client.to_html_table(),
                                                      'new_ip', ip_msg)
                error_count = 0
                ...
                if error_count > 10:
                    if self._auto_restart and self._IS_UNIX:
                        ...
                        self._email_client.send_email('ddns_error_restart', self._cloudflare_client.to_html_table())
                        ...
                        self._email_client.send_email('ddns_error_exit', self._cloudflare_client.to_html_table())
                    sys.exit(-1)
...
def main():
    ...
    不要开代理、梯子，会http连接错误
    """
    parser = argparse.ArgumentParser(description='Cloudflare DDNS: Regularly detect IP changes of home broadband and '
